...
import MarkdownIt, { Options as Options$4, PresetName, PluginSimple as PluginSimple$1, PluginWithOptions as PluginWithOptions$1, PluginWithParams as PluginWithParams$1 } from 'markdown-it';
import { PageSplitSection } from '../../types/local-search.js';
import { SiteData, MarkdownEnv, Awaitable, MarkdownLocaleOptions, CodeCopyButtonOptions, ContainerOptions, LocaleConfig, LocaleSpecificConfig, SSGContext, PageData, HeadConfig, AdditionalConfigDict, AdditionalConfigLoader, AdditionalConfig } from '../../types/shared.js';
export { HeadConfig, Header, MarkdownLocaleOptions, SiteData } from '../../types/shared.js';
...
import { ThemeRegistrationAny, LanguageInput, ShikiTransformer, CodeToHastOptions } from '@shikijs/types';
...
interface MarkdownItAsyncOptions extends Omit<Options$4, 'highlight'> {
    /**
 ...
 */
declare function resolveSiteDataByRoute(siteData: SiteData, relativePath: string, filePath?: string): SiteData;
 ...
 * 永久链接生成器函数
 */
type PermalinkGenerator = (slug: string, options: ResolvedAnchorOptions, state: StateCore, index: number) => void;
//#endregion
 ...
 */
type ResolvedAnchorOptions = AnchorOptions & Required<Pick<AnchorOptions, "getTokensText" | "level" | "slugify" | "tabIndex" | "uniqueSlugStartIndex">>;//# sourceMappingURL=index.d.ts.map
...
type AllowedAttrs = (string | RegExp)[] | AllowedAttrEntry[];
/**
 * Normalized attribute filter for fast matching
 *
 * 正则化后的属性过滤器，用于快速匹配
 */
type AttrFilter = (name: string, value: string) => boolean;
...
//#endregion
//#region src/options.d.ts
type MarkdownItAttrRuleName = "fence" | "inline" | "table" | "list" | "heading" | "hr" | "softbreak" | "blockInfo" | "blockEnd" |
...
//#region src/options.d.ts
interface EmojiPluginOptions$1 {
  /**
   * Emoji definitions
   *
   * 表情定义
...
//#endregion
//#region src/full.d.ts
declare const fullEmoji: PluginWithOptions$1<EmojiPluginOptions$1>;
//# sourceMappingURL=index.d.ts.map
...
type EmojiPluginOptions = NonNullable<Parameters<typeof fullEmoji>[1]>;
...
declare function mergeMarkdownLocales(options?: MarkdownOptions, locales?: LocaleConfig): MarkdownOptions;
declare function disposeMdItInstance(): void;
/**
 * @experimental
 */
declare function createMarkdownRenderer(srcDir: string, options?: MarkdownOptions, base?: string, logger?: Pick<Logger, 'warn'>, publicDir?: string): Promise<MarkdownRenderer>;
 ...
 */
interface SiteConfig<ThemeConfig = any> extends Pick<UserConfig<ThemeConfig>, 'markdown' | 'vue' | 'vite' | 'shouldPreload' | 'router' | 'mpa' | 'lastUpdated' | 'ignoreDeadLinks' | 'cleanUrls' | 'useWebFonts' | 'postRender' | 'buildEnd' | 'transformHead' | 'transformHtml' | 'transformPageData' | 'sitemap'> {
    /**
...
declare function mergeConfig<A extends object, B extends object>(a: A, b: B, isRoot?: boolean): A & B;
...
declare function scaffold({ root: root_, srcDir: srcDir_, title, description, theme, useTs, injectNpmScripts, addNpmScriptsPrefix, npmScriptsPrefix }: ScaffoldOptions): Promise<string>;
